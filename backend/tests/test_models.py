...
class TestCourseRow:
    ...
    def test_valid_course_with_letter_suffix(self):
        ...
        course = CourseRow(subject="ENG", number="101L", title="English Lab", units=1.0, grade="B+")
        assert course.number == "101L"

    def test_valid_transfer_credit(self):
        """Test transfer credit with TCR grade."""
        course = CourseRow(subject="MATH", number="201", title="Calculus I", units=4.0, grade="TCR")
        assert course.grade == "TCR"

    def test_valid_courses_in_progress(self):
        """Test course in progress with IP grade."""
        course = CourseRow(subject="PHY", number="205", title="Physics II", units=3.0, grade="IP")
    ...
    def test_subject_validation_invalid_cases(self):
        ...
        for invalid_subject in invalid_subjects:
            with pytest.raises(ValidationError) as exc_info:
                CourseRow(subject=invalid_subject, number="101", title="Test", units=3.0, grade="A")
    ...
    def test_number_with_single_letter(self):
        ...
        course = CourseRow(subject="BIO", number="101L", title="Biology Lab", units=1.0, grade="A")
        assert course.number == "101L"

    def test_number_xx_format(self):
        """Test XX course number format."""
        course = CourseRow(subject="CS", number="4XX", title="Test Course", units=3.0, grade="A")
        assert course.number == "4XX"

        course = CourseRow(subject="HIST", number="1XX", title="Test Course", units=3.0, grade="A")
    ...
    def test_number_invalid_format(self):
        ...
        for invalid_number in invalid_numbers:
            ...
            assert "Course number must be digits optionally followed by a single letter" in str(
                exc_info.value
            )
    ...
    def test_units_validation(self):
        ...
        for units in valid_units:
            course = CourseRow(subject="CS", number="101", title="Test", units=units, grade="A")
    ...
    def test_valid_letter_grades(self):
        ...
        for grade in valid_grades:
            course = CourseRow(subject="CS", number="101", title="Test", units=3.0, grade=grade)
    ...
    def test_title_validation(self):
        ...
        for title in valid_titles:
            course = CourseRow(subject="CS", number="101", title=title, units=3.0, grade="A")
        ...
        for invalid_title in invalid_titles:
            with pytest.raises(ValidationError) as exc_info:
                CourseRow(subject="CS", number="101", title=invalid_title, units=3.0, grade="A")
